-- Reject foreign or mismatched routine/regime identifiers before a session is
-- resumed or inserted. RLS protects row visibility, but foreign keys alone do
-- not guarantee that the referenced rows belong to the caller.
...
begin
  ...
  if not exists (
    select 1 from regimes where id = p_regime_id and user_id = v_uid
  ) then
  ...
  if not exists (
    ...
    where id = p_routine_id and regime_id = p_regime_id and user_id = v_uid
  ) then
    raise exception 'routine not found in regime';
  end if;
