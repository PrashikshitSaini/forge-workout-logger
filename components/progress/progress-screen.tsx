...
import { OverviewScreen } from "@/components/progress/overview-screen";
import { ReportsScreen } from "@/components/reports/reports-screen";
import { PageHeader } from "@/components/ui/page-header";
import { cn } from "@/lib/utils";

type View = "overview" | "history" | "reports";
    ...
    <>
      ...
      <div className="px-4 pb-5">
        <div className="grid grid-cols-3 rounded-lg border border-border bg-surface p-0.5">
          {(["overview", "history", "reports"] as const).map((option) => (
            <button
              key={option}
              type="button"
      ...
      {view === "overview" ? <OverviewScreen /> : view === "history" ? <HistoryScreen embedded /> : <ReportsScreen embedded />}
    </>
  );
}
