  ...
  if (routine.routine_exercises.length === 0) {
    return (
      <div className="px-4 py-10 text-center">
        <p className="text-muted">This routine has no exercises yet.</p>
        <Link href={`/routines?routine=${routine.id}`} className="mt-3 inline-block">
          <Button variant="secondary">Add exercises</Button>
        </Link>
      </div>
    );
  }
